...
import { createFormatter, shouldUseColor } from "./formatter.js";
...
async function readStdinToTempFile() {
...
function createFormatterFromOptions(globalOptions) {
    const formatOptions = {
...
function validateFile(filePath, allowMissing = false) {
...
function runFastgron(args, formatter, onError) {
...
program
    ...
    runFastgron([inputFile], formatter, (code) => {
...
program
    ...
    runFastgron(["-u", inputFile], formatter, (code) => {
        // Clean up temporary file if created
        if (isTemporary) {
            try {
                require('fs').unlinkSync(inputFile);
...
program
    ...
    .action(async (fileOrTerm, term, options, command) => {
    const globalOptions = command?.parent?.opts() || {};
    const formatter = createFormatterFromOptions(globalOptions);
    let inputFile;
    let searchTerm;
